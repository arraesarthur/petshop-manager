...
export const ClienteListagem = () => {
  ...
  return (
    <div className='container mx-auto py-10 max-w-7xl px-4 sm:px-6 lg:px-8 space-y-6'>
      <div className='flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-pink-100 pb-5'>
        <div>
          ...
          <p className='text-muted-foreground mt-1'>
            Gerencie o cadastro, endereços e informações de contato dos
            clientes.
          </p>
        </div>
      ...
      <div className='flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between bg-pink-50/40 border border-pink-100/60 p-4 rounded-xl shadow-sm'>
        <div className='relative w-full max-w-sm flex items-center'>
          ...
          <Input
            ...
            className='pl-9 bg-background border-pink-200/80 focus-visible:ring-pink-400 focus-visible:border-pink-400'
          />
          {loading && (
      ...
      <div className='bg-card rounded-xl border border-pink-100/80 shadow-xs overflow-hidden'>
        <DataTable
          ...
          onRowClick={cliente => navigate(`/editar/clientes/${cliente.id}`)}
        />
      </div>
    </div>
  )
